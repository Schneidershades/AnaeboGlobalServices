	...
	$.ajax({
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td >'+
					'<div class="form-group row" style="padding-left:10px;padding-right:10px">'+
					'<input type="number" name="empties[]" id="empties'+count+'" onkeyup="getEmpties('+count+')" autocomplete="off" class="form-control" min="1" />'+
					'</div>'+
				'</td>'+
				'<td>'+
					'<div class="form-group row" style="padding-left:10px;padding-right:10px">'+
...
function getProductData(row = null) {
	if(row) {
		...
		if(productId == "") {
			...
			$.ajax({
				url: '/url/js/api/fetchStock/'+productId,
				type: 'get',
				dataType: 'json',
				success:function(response) {
					// setting the rate value into the rate input field
					
					$("#rate"+row).val(response.bulkbreaker);
					$("#rateValue"+row).val(response.bulkbreaker);

					$("#quantity"+row).val(1);

					var total = Number(response.bulkbreaker) * 1;
...
function getTotal(row = null) {
	if(row) {
		var total = Number($("#rate"+row).val()) * Number($("#quantity"+row).val()) - Number($("#empties"+row).val() * 1000);
		total = total.toFixed(2);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total);
