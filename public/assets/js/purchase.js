...
function addRow() {
	...
	var tableLength = $("#item_table tbody tr").length;
	...
	if(tableLength > 0) {		
		tableRow = $("#item_table tbody tr:last").attr('id');
		arrayNumber = $("#item_table tbody tr:last").attr('class');
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				'<td>'+
					...
					'<select class="form-control js-example-basic-single col-sm-12 select-stock" name="productName[]" id="productName'+count+'" onchange="getProductData('+count+')" >'+
						'<option value="">~~SELECT STOCK~~</option>';
						// console.log(response);
						$.each(response, function(index, value) {
							tr += '<option value="'+value.id+'">'+value.abbr+'</option>';							
						});
				...
				'<td>'+
					'<div class="form-group row" style="padding-left:10px;padding-right:10px">'+
						'<input type="number" name="rate[]" id="rate'+count+'" autocomplete="off" class="form-control" required/>'+
				...
				'</td>'+
				'<th>'+
					'<button class="btn btn-default removeProductRowBtn" type="button" onclick="removeProductRow('+count+')"><i class="icofont icofont-minus"></i></button>'+
				'</th>'+
			'</tr>';
			if(tableLength > 0) {							
				$("#item_table tbody tr:last").after(tr);
			} else {				
...
function getProductData(row = null) {
	if(row) {
		...
		if(productId == "") {
			...
			$("#rate"+row).keyup(function(){
				var unit_purchased = $(this).val();
				if (unit_purchased == 0 || unit_purchased == '' ) {
					unit_purchased = 0;
				}

				$("#quantity"+row).bind('keyup mouseup', function(){
				    $("#quantity"+row).val();
				    var total = Number(unit_purchased) * $("#quantity"+row).val();
